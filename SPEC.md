...
- Build-authored code comments and docstrings use English only, regardless of implementation language; stable ledger IDs may be cited in commit & handoff prose
...
implementation-artifact: Build-authored comments/docstrings → English only + intent & rationale ∀ authored non-generated function; commit subject `build: <goal>` | `fix: <root cause>`
...
V11: Distill requires existing root `SPEC.md`, readable format & no task `~`; live implementation reference to a ledger ID ≠ blocker, since surviving IDs are unchanged by compaction
...
V16: every authored/materially changed non-generated function includes intent + rationale comment/docstring, English only, in that language's normal syntax; feature commit = `build: <goal>`; defect commit = `fix: <root cause>`; stable ledger ID permitted in commit & handoff prose
...
T14|.|Drop the SPEC-identifier ban from the Build artifact contract, keeping English comments & intent rationale|V16,I.implementation-artifact
